...
class QPPlannerCVAE:
    ...
    def step(self, q, x_goal, obs_objs, noise_factor=0.0):
        ...
        if x_goal.dim() == 0 or x_goal.shape[-1] != 2:
            raise ValueError(f"x_goal must have shape (2,), got {x_goal.shape}")
        
        # at every step, sample gradient from CVAE, and cultivate coresponding q_goal
        num_x_goal = x_goal.shape[0]
        num_q = q.shape[0]
        # expand q to match x_goal
        q_expanded = q.unsqueeze(0).expand(num_x_goal, num_q)  # (num_x_goal, dof)
        gradient, _, _, distance = self.cvae_model.compute_vector_field(
            torch.cat([q_expanded, x_goal], dim=1), n_samples=self.n_cvae_samples
        )
        gradient = gradient[0, :self.dof]  # (dof,)
        gradient_norm = torch.norm(gradient) + 1e-6
        gradient = gradient / gradient_norm  * distance  # Scale gradient by average distance
        q_goal_candidate = q - gradient   # Move opposite to gradient
        q_goal = q_goal_candidate.clone().detach()
        print(f"  Sampled q_goal from CVAE: {q_goal.detach().cpu().numpy()}")
...
def plot_qp_planning_cvae(obj_lists, filename, cvae_model_path, cdf_model_path, 
    ...
    data_dim = 1  # 2D robot has 2 joints
    condition_dim = 2+2
    cvae_model = CVAE(data_dim=data_dim, condition_dim=condition_dim, latent_dim=32, hidden_dims=[256, 128]).to(device)
    ...
    # sample num_x_goal target points from targ_objs if not provided
    num_x_goal = 20
    if x_goal is None:
        x_goal = torch.tensor([], dtype=torch.float32).to(device)
        if len(targ_objs) == 0:
            raise ValueError("No target objects provided for planning.")
        for targ in targ_objs:
            sampled_pts = targ.sample_surface(num_x_goal)  # (1, 2)
            x_goal = torch.cat([x_goal, sampled_pts.to(device)], dim=0)
        print(f"Sampled {x_goal.shape[0]} target points from target objects.")    
    else:
        x_goal = torch.tensor(x_goal, dtype=torch.float32).to(device)
        print(f"Using provided target points: {x_goal.detach().cpu().numpy()}")
    # 创建QP规划器
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_cvae_samples', type=int, default=10,
    ...
    scene_6_object = [
        Box(center=torch.tensor([1.0, -1.5]).to(device), w=0.5, h=0.5, attract=False, device=device),
        Box(center=torch.tensor([1.0, -2.5]).to(device), w=0.5, h=0.5, attract=False, device=device)
    ]
    scene_6_target = [
        Box(center=torch.tensor([1.5, -1.5]).to(device), w=0.5, h=0.5, attract=True, device=device),
        Box(center=torch.tensor([1.5, -2.5]).to(device), w=0.5, h=0.5, attract=True, device=device)
    ]
    # Select scene
